...
def get_latest_video(username):
    ...
    embed_url = f"https://www.tiktok.com/oembed?url=https://www.tiktok.com/@{username}"
    try:
        ...
        for username in TIKTOK_USERS:
            ...
            try:
                ...
                if latest_id is None:
                    print(f"Could not fetch video for {username}, skipping...")
                    continue

                previous_id = self.last_video.get(username)
                print(f"{username}: previous={previous_id}, current={latest_id}")

                if latest_id != previous_id:
                    if previous_id is not None:
                        # This is a NEW video (not the first time we've seen this user)
                        display = USER_NAMES.get(username, username)
                        url = f"https://www.tiktok.com/@{username}/video/{latest_id}"
                        message = f"@everyone {display} just posted a new video, go check it out!! {url}"
                        ...
                        print(f"First time seeing {username}, storing video ID without notification")
